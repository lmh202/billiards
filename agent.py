...
class NewAgent(Agent):
    ...
    def __init__(self, checkpoint_path=None, device="cpu"):
        ...
        参数：
            checkpoint_path: 模型检查点路径（如果为None则使用随机策略）
        """
        super().__init__()
        ...
        if checkpoint_path is not None and os.path.exists(checkpoint_path):
            ...
            print(f"[NewAgent] 未提供检查点或文件不存在，使用随机策略")
    ...
    def decision(self, balls=None, my_targets=None, table=None):
        ...
        """
        if balls is None:
            print(f"[NewAgent] 未收到球状态信息，使用随机动作")
            return self._random_action()
        ...
        try:
            import torch
            from train.muzero_train import encode_state, action_index_to_dict
            
            # 检查目标球，如果已全部清空则目标为8号球
            remaining_own = [bid for bid in my_targets if balls[bid].state.s != 4]
            if len(remaining_own) == 0:
                my_targets = ["8"]
                print("[NewAgent] 目标球已全部清空，切换目标为8号球")
            ...
            print(f"[NewAgent] 决策过程出错，使用随机动作。错误: {e}")
            import traceback
            traceback.print_exc()
            return self._random_action()